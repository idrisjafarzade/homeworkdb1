    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Delete(int? id)
        {
           ...
           // return RedirectToAction(nameof(Basket));
            ...
            return PartialView("_PartialBasket", basketViewModels);
        ...
        public IActionResult DeleteCount(int? Id)
        {
            ...
            var basketViewModels = JsonConvert.DeserializeObject<List<BasketVM>>(basket);
            var Exist = basketViewModels.FirstOrDefault(x => x.Id == Id);
            if (Exist.Count == 1)
            {
                basketViewModels.Remove(Exist);
            }
            else
            {
              Exist.Count--;
        ...
        public async Task<IActionResult> searched(string searchedProduct)
        {
            var product = await _appDbContext.products.Where(x => x.Name.ToLower() == searchedProduct.ToLower()).ToListAsync();
            return PartialView("_SearchedPartial",product);
        }
    }
}
